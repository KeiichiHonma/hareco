...
class CI_Image_lib {
    ...
    var $command_create_thumb = FALSE;
    var $command_issue        = FALSE;
    var $balloooooon_create_thumb        = FALSE;//mangahack only
    var $balloooooon_thumb_marker        = '_m_thumb';//mangahack only
    var $balloooooon_thumb_width                = '';
    var $balloooooon_thumb_height                = '';
    ...
    var $full_src_path        = '';
    var $full_dst_path        = '';
    var $balloooooon_full_thumb_path        = '';
    ...
    function initialize($props = array())
    {
        ...
        if($this->command_issue){
            $this->balloooooon_full_thumb_path = $this->dest_folder.'thumb'.$file_ext;
        }else{
            $this->balloooooon_full_thumb_path = $this->dest_folder.$filename.$this->balloooooon_thumb_marker.$file_ext;
        }
    ...
    function delete_oldfile($old_file_name = '',$new_file_name = '')
    {
        if($old_file_name != '' && $new_file_name != ''){
            $old_fileinfo = pathinfo($old_file_name);
            $new_ext = pathinfo($new_file_name,PATHINFO_EXTENSION);
            if($old_fileinfo['extension'] != $new_ext){
                @unlink($this->source_folder.$old_file_name);
                if($this->balloooooon_create_thumb){
                    $old_balloooooon_full_thumb_path = $this->source_folder.$old_fileinfo['filename'].$this->balloooooon_thumb_marker.'.'.$old_fileinfo['extension'];
                    if(file_exists($old_balloooooon_full_thumb_path)) @unlink($old_balloooooon_full_thumb_path);
                }
    ...
    function image_process_gd($action = 'resize')
    {
        ...
        $v2_override = FALSE;
if(!$this->command_create_thumb || $this->command_issue){
...
if(!$this->command_create_thumb || $this->command_issue){
        ...
        //  Create the thumb image handle
        //mangahack only
        if ($this->balloooooon_create_thumb && !$this->command_issue)
        {
            if (@copy($this->full_src_path, $this->balloooooon_full_thumb_path))
            {
                @chmod($this->balloooooon_full_thumb_path, FILE_WRITE_MODE);
            }
            if ( ! ($src_thumb_img = $this->image_create_gd($this->balloooooon_full_thumb_path)))
            {
                return FALSE;
            }
            $this->balloooooon_thumb_image_reproportion();
            $dst_thumb_img = $create($this->balloooooon_thumb_width, $this->balloooooon_thumb_height);
            ...
            //$copy($dst_thumb_img, $src_thumb_img, 0, 0, $this->x_axis, $this->y_axis, $this->balloooooon_thumb_width, $this->balloooooon_thumb_height, $this->orig_width, $this->orig_height);
            $copy($dst_thumb_img, $src_thumb_img, 0, 0, $this->x_axis, $this->y_axis, $this->balloooooon_thumb_width, $this->balloooooon_thumb_height, $this->width, $this->height);
            //  Show the image
            if ($this->dynamic_output == TRUE)
            {
                $this->image_display_gd($dst_thumb_img);
            }
            else
            {
                // Or save it
                if ( ! $this->image_save_gd($dst_thumb_img,TRUE))
                {
                    return FALSE;
                }
            }
...
if(!$this->command_create_thumb || $this->command_issue){
     ...
     * @return    bool
     */
    function image_save_gd($resource,$is_balloooooon_thumb = FALSE)
    {
        switch ($this->image_type)
        {
            case 1 :
                        ...
                        if ( ! @imagegif($resource, $is_balloooooon_thumb ? $this->balloooooon_full_thumb_path : $this->full_dst_path))
                        {
            ...
            case 2    :
                        ...
                        if ( ! @imagejpeg($resource, $is_balloooooon_thumb ? $this->balloooooon_full_thumb_path : $this->full_dst_path, $this->quality))
                        {
                            $this->set_error('imglib_save_failed');
                            return FALSE;
                        }
            ...
            case 3    :
                        ...
                        if ( ! @imagepng($resource, $is_balloooooon_thumb ? $this->balloooooon_full_thumb_path : $this->full_dst_path))
                        {
                            $this->set_error('imglib_save_failed');
                            return FALSE;
                        }
