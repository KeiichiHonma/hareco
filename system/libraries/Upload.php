...
class CI_Upload {
     ...
     * @return    bool
     */
    public function do_upload($field = 'userfile',$old_file_name = '')
    {
    // Is $_FILES[$field] set? If not, no reason to continue.
        ...
        $this->file_ext     = strtolower($this->get_extension($this->file_name));
        ...
        // Are the image dimensions minimum allowed size.
        // Note: This can fail if the server has an open_basdir restriction.
        ...
        if ($this->overwrite == FALSE)
        {
            ...
            if($old_file_name != ''){
                $fileinfo = pathinfo($old_file_name);
                $this->upload_path = $fileinfo['dirname'].'/';//oldファイルがある場合は、自動パス入力
                if($this->file_ext != $fileinfo['extension']){
                    $this->file_name = $fileinfo['filename'].$this->file_ext;//名前そのまま、拡張子だけ変更
                }else{
                    $this->file_name = $fileinfo['basename'];//上書き
                }
            }
    ...
    public function is_allowed_dimensions_min()
    {
        ...
        if (function_exists('getimagesize'))
        {
            ...
            if ($this->min_width < $D['0'] || $this->min_height < $D['1'])
            {
                return TRUE;
            }
