...
class Tank_auth
{
                ...
                if ($hasher->CheckPassword($password, $user->password)) {        // password ok
                    ...
                    if ($user->banned == 1) {                                    // fail - banned
                        ...
                        $this->ci->session->set_userdata(array(
                                ...
                                //'language'=>$this->ci->config->item('language_min')
                        ));
    ...
    function logout()
    {
        ...
        unset($this->ci->session->userdata['cart_promotion_ids']);
        $this->ci->session->sess_write();
        // See http://codeigniter.com/forums/viewreply/662369/ as the reason for the next line
        $this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'status' => '', 'profile_image' => ''));
     ...
     * @return    bool
     */
    function is_console_logged_in($activated = TRUE)
    {
        if( $this->ci->session->userdata('user_id') != 1 ) return FALSE;
        return $this->ci->session->userdata('status') === ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);
    }
    ...
    {
        return $this->ci->session->userdata('profile_image');
    }
    ...
    function create_user($userData, $email_activation)
    {
        if (!$this->ci->users->is_email_available($userData['email'])) {
            ...
            $this->ci->config->item('phpass_hash_strength', 'tank_auth'),
            $this->ci->config->item('phpass_hash_portable', 'tank_auth'));
            $hashed_password = $hasher->HashPassword($userData['password']);
            ...
            $data = array(
                ...
                'need_notify'    => $userData['need_notify'],
     ...
     * @return    array
     */
    function set_new_email_no_password($new_email)
    {
        $user_id = $this->ci->session->userdata('user_id');
