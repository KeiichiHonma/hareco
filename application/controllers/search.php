...
class Search extends MY_Controller
{
    ...
    function index()
    {
        ...
        $data['tagsData'] = $this->Tag_model->getTagsByTagNames($tags);

        if(!empty($data['tagsData'])){
            if(count($data['tagsData']) == 1){
                switch ($data['tagsData'][0]->tag_type){
                    case 0://area
                        $url = $data['date'] != '' ? 'area/date/'.$data['tagsData'][0]->object_id.'/'.$data['date'] : 'area/show/'.$data['tagsData'][0]->object_id;
                    break;
                    case 1://spring
                        $url = $data['date'] != '' ? 'spring/date/'.$data['tagsData'][0]->object_id.'/0/'.$data['tagsData'][0]->area_id.'/'.$data['date'] : 'spring/show/'.$data['tagsData'][0]->object_id;
                    break;
                    case 3://airport
                        $url = $data['date'] != '' ? 'airport/date/'.$data['tagsData'][0]->object_id.'/'.$data['date'] : 'airport/show/'.$data['tagsData'][0]->object_id;
                    break;
                    case 4://leisure
                        $url = $data['date'] != '' ? 'leisure/date/'.$data['tagsData'][0]->object_id.'/'.$data['date'] : 'leisure/show/'.$data['tagsData'][0]->object_id;
                    break;
                }
                redirect($url);
            }else{
                ...
                $data['bodyId'] = 'ind';
                $this->load->view("search/keyword/candidate", array_merge($this->data,$data));
            }
        }else{
