...
*/
        if (!$this->tank_auth->is_console_logged_in()) {                                // not logged in or not activated
            print json_encode( array('result'=>'error','message'=>'deny') );
            die();
        }
        $is_success = TRUE;
        ...
        if($count == 0){
            print json_encode( array('result'=>'error','message'=>'no file') );
            die();
        }elseif($count == 1){
            $function = 'validate';
        }else{
            $function = 'multiple_validate';
        }
        
        //if ( TRUE === ( $validateResult = $this->uploadfile_validate->multiple_validate() ) && count($_FILES) > 0 ) {
        if ( TRUE === ( $validateResult = call_user_func(array($this->uploadfile_validate, $function),'files') ) ) {
            ...
            if ( TRUE === ( $validateResult = $this->validate($_FILES['files']) ) ) {
                if (count($this->uploadFiles) == 0) {
                    ...
                    foreach ($this->uploadFiles as $key=> $file) {
                        ...
                        //$upload_files[] = array('old'=>$file['name'],'new'=>$newFilename);
                        ...
                        $result['class'] = str_replace(array('.','_','-'),array('','','_'),$file['name']);
                        $result['new'] = site_url($newFilename);
                    ...
                    if(!$is_success){
                        ...
                        print json_encode( $result );//for javascript
                        die();
                    }
                }
            }
        ...
        if(isset($validateResult)){
            print json_encode( array('result'=>'error','message'=>$validateResult) );
            die();
        }
            print json_encode( array('result'=>'error','message'=>'deny') );
            die();
    ...
    private function check_file_size_rect($files)
    {
        ...
        foreach($files as $key =>  $file) {
            ...
            $size = $size + $file['size'];
            //type
            if ( ! in_array($file['type'], $this->config->config['upload_editor_image_allowed_types'])) {
